locals {
  ansible_vars = templatefile("./ansible-vars.json.tpl", {
    ...
    db_host           = "aws_db_instance.sre_db.endpoint"
...
resource "aws_instance" "sre_main" {
  ...
  provisioner "local-exec" {
    command = "printf '\nubuntu@${data.aws_eip.sre_eip.public_ip}' >> hosts.txt && aws ec2 wait instance-status-ok --instance-ids ${self.id} --region ${var.region}"
  }
...
resource "null_resource" "ssh" {
  provisioner "remote-exec" {
    ...
    connection {
      ...
      host        = data.aws_eip.sre_eip.public_ip
    }
  }
...
data "aws_eip" "sre_eip" {
  id = var.allocation_id
}

resource "aws_eip_association" "sre_eip_assoc" {
  count         = var.main_instance_count
  instance_id   = aws_instance.sre_main[count.index].id
  allocation_id = data.aws_eip.sre_eip.id
}

output "IP" {
  value = data.aws_eip.sre_eip.public_ip
}
...
output "instance_ips" {
  value = { for i in aws_instance.sre_main[*] : i.tags.Name => "${i.public_ip}:3000" }
}
